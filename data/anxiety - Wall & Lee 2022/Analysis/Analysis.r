...
MASQ.v<-c(1,a1,b1,c1,d1,e1,f1,g1,h1,i1,j1,k1,l1)
...
"STAI-T.v"<-c(a2,1,b2,c2,d2,e2,f2,g2,h2,i2,j2,k2,l2)
...
"STAI-S.v"<-c(a3,b3,1,c3,d3,e3,f3,g3,h3,i3,j3,k3,l3)
...
"TMAS.v"<-c(a4,b4,c4,1,d4,e4,f4,g4,h4,i4,j4,k4,l4)
...
"STICSA.v"<-c(a5,b5,c5,d5,1,e5,f5,g5,h5,i5,j5,k5,l5)
...
"SCL-90-R.v"<-c(a6,b6,c6,d6,e6,1,f6,g6,h6,i6,j6,k6,l6)
...
"ZungSAS.v"<-c(a7,b7,c7,d7,e7,f7,1,g7,h7,i7,j7,k7,l7)
...
"CC-DAS.v"<-c(a8,b8,c8,d8,e8,f8,g8,1,h8,i8,j8,k8,l8)
...
"4DSQ.v"<-c(a9,b9,c9,d9,e9,f9,g9,h9,1,i9,j9,k9,l9)
...
"HADS.v"<-c(a10,b10,c10,d10,e10,f10,g10,h10,i10,1,j10,k10,l10)
...
"DASS.v"<-c(a12,b12,c12,d12,e12,f12,g12,h12,i12,j12,1,k12,l12)
...
"BAI.v"<-c(a13,b13,c13,d13,e13,f13,g13,h13,i13,j13,k13,1,l13)
...
"CAS.v"<-c(a14,b14,c14,d14,e14,f14,g14,h14,i14,j14,k14,l14,1)

# Create table
M = matrix(nrow=13, ncol=13) 
colnames(M) <- c("MASQ",	"STAI-T",	"STAI-S",	"TMAS",	"STICSA", "SCL-90-R", "ZungSAS", 
                 "CC-DAS", "4DSQ", "HADS", "DASS", "BAI", "CAS")
...
colMeans(M)
mean(colMeans(M)) #0.236

length1<-c(28,20,20,50,42,29,20,9,28,7,14,21,25) # length of original questionnaires
length2<-c(25,14,14,31,20,25,25,8,20,8,19,16,16) # items in analysis per scale; MASQ captures 25 items

cor(length1, colMeans(M)) #0.276
cor(length2, colMeans(M)) #0.577
...
pal1 <- c("#999999", "#AA00AA", "#FF55FF", "#5555FF", "#0000AA", "#00AAAA", 
...
a<- ggplot(d, aes(x=S, y=Scales2, group=S, color=as.factor(Scales), shape=Type, rev=F)) +
  ...
  geom_hline(yintercept = 1:13, colour = "grey80", size = .2) +
  ...
  theme(axis.text.x = element_text(angle = 360/(2*pi)*rev( pi/2 + seq( pi/60, 2*pi-pi/60, len=60)) + 
        ...
        legend.position = "top") +
  scale_y_continuous(limits=c(-4,13), expand=c(0,0), breaks=1:13, labels=d[, levels(Scales)]) +
